...
function reply(query) {
  ...
  const lines = [r.fuzzy ? `没找到完全匹配的，这 ${r.total} 首曲名比较接近，看看是不是你要找的♪` : r.total === 1 ? "" : `找到 ${r.total} 首：`];
  for (const song of r.matches) {
    lines.push(`《${song.meta.name}》${song.meta.is_deleted ? "（已删除）" : ""}`);
    lines.push(["BAS", "ADV", "EXP", "MAS", "LUN"].filter(d => song[d]?.has_chart)
      .map(d => `${d} ${song[d].const_status === "known" && song[d].const != null ? song[d].const : "定数未知"}`).join(" / "));

  }
  if (r.pages > 1) lines.push(`第 ${r.page}/${r.pages} 页；${r.page < r.pages ? `下一页：/搜索歌曲 ${r.repeatQuery} --page ${r.page + 1}` : "已到最后一页"}`);
