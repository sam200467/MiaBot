    ...
    adapter: {
      ...
      ability: () => commands
        ? "运行时实际能力：你正在 QQ 里回复消息。"
          ...
          // 版本。不写这句，路由万一兜到闲聊，人设会答「能拿哪些牌子列不全」——
          // 那是把程序答得上来的问题推回给用户（实测群里就是这句）。
          + "（用户问有哪些版本牌子、或说不出版本名时照样调用，程序会把全部版本列出来，"
          + "不要回答「列不全」；绑定只在真正出图时才需要）、生成单曲全难度成绩图、"
          + "生成单张谱面分数线分析图、查定数表、生成等级成绩长图、算单曲 Rating、查看某首歌的全部别名、"
          + "按别名或部分曲名查歌、给歌曲加别名、看运行状态（只在用户明确问你在不在线、"
