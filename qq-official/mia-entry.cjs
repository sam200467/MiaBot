...
function createMiaBot(config, deps = {}) {
  ...
  const chat = createChat(settings, {
    ...
    // 美亚不联网 —— 不传 proxyUrl，dispatcher 就不会建
  }, {
    log,
    ...
    adapter: {
      ...
      //
      // 最后那段「问事实 vs 要你用道具」是设计上刻意加的：美亚是角色，不是数据源，
      // 别人随口问她一个定数，她要照人设说记不住；只有明确让她「查一下」才动手。
      // 这条规则只能写在这儿 —— chat.cjs 的 actionRule 是引擎侧拼的，前端改不了。
      // 上线后要拿真模型实测校准；实在不稳就退回「闲聊一律不查、只指路」。
      ability: () => commands
        ? "运行时实际能力：你正在 QQ 里回复消息。"
          ...
          + "「帮我查一下 id870」直接查单曲成绩就行（单曲图不分难度），别反过来要难度；"
          + "用户说的指令叫法（比如 b110 就是 B50+N10+P50 的分表）照办就行，"
          + "不要质疑人家「是不是想说别的」。能办就办，别把活儿推回去。"
        : "运行时实际能力：你正在 QQ 里回复消息，只能说话，没有查分、联网或读图能力。可以按语境发送你的表情图。",
      ...
      ...(commands ? {
        ...
        runAction: async (action, message, result) => {
          ...
          const invented = action.name === "calculate"
            ? commands.hasInventedEnum(action.query, message.content) : [];
          if (invented.length) {
        ...
        alias: {
          resolve: (word, game) => core.resolveAliasTitle(word, game || ""),
