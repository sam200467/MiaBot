...
// 命令总数超过平台 20 项上限；/取消 和 /状态仍可手动输入、仍在帮助中，
// 但不占发现面板的位置。手动传 exclude 时以调用方为准。
const DEFAULT_EXCLUDE = Object.freeze(["cancel", "status"]);
...
function panelItems(options = {}) {
  const exclude = new Set(options.exclude === undefined ? DEFAULT_EXCLUDE : options.exclude);
  const items = [];
