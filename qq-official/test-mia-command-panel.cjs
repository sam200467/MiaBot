...
test("面板元素数量不超过平台上限", () => {
  ...
  assert.equal(items.length, Object.keys(COMMANDS).length, "默认应当把每条命令都放进去");
});
...
test("version 由内容决定：内容不变就不该重复提交", () => {
  ...
  const c = panel.buildPanel({ exclude: ["cancel"] });
...
test("还没建过：调 POST /v2/panels，且只挂指定群", async () => {
  ...
  assert.equal(create.body.panel.items.length, Object.keys(COMMANDS).length);
...
test("已经建过但内容变了：用 PUT 改内容（不动已关联的群）", async () => {
  ...
  assert.equal(put.body.panel.items.length, Object.keys(COMMANDS).length);
});
