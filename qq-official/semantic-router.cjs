...
function validateDecision(value, specs, targets = []) {
  ...
  // 这种能答的问题推回给用户 —— 实测群里问「总共有哪些牌子可以拿」就卡在这里，
  // 模型只好自己编一句「能拿哪些牌子目前没法列全」。
  if (["songsearch", "song", "level", "constant", "chartinfo", "calculate", "aliases", "whatis", "aliasadd"].includes(a.name) && !query) {
