...
const gameOfToken=token=>/音|ongeki/i.test(token)?'ongeki':/中二|chunithm/i.test(token)?'chunithm':'maimai';
// 一段话里**唯一**提到的游戏；同时提到多款、或一款都没提时返回 null。
// 定数裁决层要用它消歧：同一首歌可能同时收录在多款游戏里（Love & Justice 在音击是
// MAS 14.7、在中二是 MAS 15.2），只按「数值最近」挑会挑到别的游戏上去。
function gameInText(text){
 const games=new Set();
 for(const m of String(text??'').matchAll(GAME_TOKENS))games.add(gameOfToken(m[0]));
...
function lookup(knowledge,query){
  ...
  // 版本与 BPM：术语层认出来的版本俗称（「真超檄」「堇代」）最后要落到这里才有用——
  // 快照的 version 字段就是版本名，组合称呼摊平成一组版本名。
  const versions=query.version==null?[]:(Array.isArray(query.version)?query.version:[query.version]).map(v=>normalize(String(v)));
  const bpm=query.bpm==null||query.bpm===''?null:(Number.isFinite(Number(query.bpm))?Number(query.bpm):NaN);
  ...
  const opponents=query.game==='ongeki'&&title&&knowledge.characters
    ...
    :[];
  return {game:query.game,source:d.source,scope:d.scope,updatedAt:d.updatedAt,fetchedAt:d.fetchedAt,total,offset,charts:rows,opponents,
