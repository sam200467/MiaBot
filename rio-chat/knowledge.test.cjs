...
const {lookup,loadKnowledge,constrainQuery,gameInText}=require('./knowledge.cjs');
...
test('gameInText 只在唯一提到一款游戏时给线索',()=>{
 assert.equal(gameInText('中二有没有比较水的14+'),'chunithm');
 assert.equal(gameInText('音击的 BATTLE NO.1 怎么打'),'ongeki');
 assert.equal(gameInText('舞萌DX 13+ 简单点'),'maimai');
 // 同时提到多款、或一款都没提 → 没有线索。定数裁决层宁可不动，也不猜游戏。
