...
test("probabilities have no image cooldown or dedup",()=>{
...
test("message authorization, direct mention and dedup",async()=>{
...
test("history isolated by channel/user and bounded",async()=>{
...
test("stop teasing persists beyond history expiry and reset",async()=>{
...
test("concurrent requests do not race, stop applies in flight",async()=>{
...
test("explicit image, permission fallback, repeated images allowed",async()=>{
...
test("API sends requested model, JSON, nonthinking; rejects invalid data and hides raw errors",async()=>{
