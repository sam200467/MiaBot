...
const {loadKnowledge,lookup,answerRule,toolRule,constrainQuery,gameInText}=require("./knowledge.cjs");
...
function loadSettings(root) {
  ...
  return {c,persona,examples,manifest,root,facts,characterName,research,knowledge:loadKnowledge(facts),stories:loadStories(facts),profiles:loadProfiles(facts),terms:loadTerms(facts),search:loadSearch(root)};
}
...
async function requestReply(settings, messages, options={}) {
  ...
  const system=voice+"\n\n"+ability+jsonRule+INTENT_RULE+(plan.required?'':actionRule)+answerRule+personalRule+toolRule(settings.research===false?null:settings.knowledge)+loreRule(settings.stories)+profilesRule(settings.profiles)+termsRule(settings.terms)+searchRule+researchRule+(plan.required?'\n本轮程序已判定必须先检索。攻略回复可以分点说明，不受3至5句限制。不要用“需要个人成绩”拒绝一般推荐；仅个性化排序需要成绩。回答结构：先说明经证据确认的游戏/曲名或目标→直接给推荐/有依据的操作建议→简短说明资料不足或适用版本。没有该曲具体攻略时可以给明确标为通用练习的建议和真实手元链接，但不声称该曲存在某种配置。不要向用户解释内部额度或检索次数。':'');
