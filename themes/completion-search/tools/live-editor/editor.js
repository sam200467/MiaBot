...
(() => {
  ...
  function clearRuntimeLayout(frameDocument) {
    frameDocument.querySelectorAll("[style]").forEach((element) => {
      if (element.style.fontSize) element.style.removeProperty("font-size");
    });
    frameDocument.getElementById("level-groups")?.style.removeProperty("--completion-compact");
  }
  ...
  function applyCssNow() {
    ...
    requestAnimationFrame(() => frameWindow.__FIT_COMPLETION_THEME__?.());
  }
  ...
  async function prepareFrame() {
    ...
    try {
      ...
      frameReady = true;
      frameWindow.__FIT_COMPLETION_THEME__?.();
  ...
  function refreshPreview() {
    ...
    elements.frame.src = "/completion-search/renderer/theme.html?sample=" + sample + "&liveEditor=1&t=" + Date.now();
  }
  ...
  function setZoom(percent) {
    const value = Math.max(15, Math.min(100, Number(percent) || 40));
  ...
  async function saveCss() {
    ...
    try {
      ...
      markDirty();
      showToast("已保存到 completion-search/renderer/theme.css");
    } catch (error) {
      markDirty();
      showToast(error.message || String(error), true);
    } finally {
