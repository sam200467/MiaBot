...
const SAMPLES = Object.freeze({
  template: path.join(THEME_DIR, "examples", "preview-data.js"),
  stress: path.join(THEME_DIR, "examples", "preview-stress.js"),
});
...
function sampleName(value) {
  return Object.hasOwn(SAMPLES, value) ? value : "template";
}
...
async function handleRequest(request, response) {
  ...
  if (pathname === "/__preview_data__.js" && request.method === "GET") {
    const selectedSample = sampleName(requestUrl.searchParams.get("sample") || "template");
    serveFile(response, SAMPLES[selectedSample]);
    return;
  }

  if (pathname === "/completion-search/renderer/theme.html" && request.method === "GET") {
    const selectedSample = sampleName(requestUrl.searchParams.get("sample") || "template");
...
server.listen(requestedPort(), HOST, () => {
  ...
  console.log("");
  console.log("牌子完成度模板 CSS 实时调试器已启动");
  console.log("地址：" + url);
