...
const SAMPLE_PATH = path.join(THEME_DIR, "examples", "preview-data.js");
...
async function handleRequest(request, response) {
  ...
  if (pathname === "/api/meta" && request.method === "GET") {
    ...
    sendJson(response, 200, {
      ...
      cssFile: CSS_PATH,
      modifiedAt: stat.mtime.toISOString(),
      sample: "完整 B50 + N10 + P50",
    });
    return;
  }

  if (pathname === "/__preview_data__.js" && request.method === "GET") {
    serveFile(response, SAMPLE_PATH);
    return;
  }

  if (pathname === "/rating-chart/renderer/theme.html" && request.method === "GET") {
...
server.listen(requestedPort(), HOST, () => {
  ...
  console.log("");
  console.log("B50 / N10 / P50 模板 CSS 实时调试器已启动");
  console.log("地址：" + url);
