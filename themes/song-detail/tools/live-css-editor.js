...
const SAMPLES = Object.freeze({
  normal: path.join(THEME_DIR, "examples", "preview-data.js"),
  lunatic: path.join(THEME_DIR, "examples", "preview-lunatic-only.js"),
  long: path.join(THEME_DIR, "examples", "preview-long-title.js"),
});
...
function sampleName(value) {
  return Object.hasOwn(SAMPLES, value) ? value : "normal";
}

function serveThemeHtml(response, selectedSample) {
  ...
  const result = html
    .replace('<script src="preview-data.js"></script>', editorMarker + "\n  <script src=\"" + previewScript + "\"></script>");
...
async function handleRequest(request, response) {
  ...
  if (pathname === "/__preview_data__.js" && request.method === "GET") {
    const selectedSample = sampleName(requestUrl.searchParams.get("sample") || "normal");
    serveFile(response, SAMPLES[selectedSample]);
    return;
  }

  if (pathname === "/song-detail/renderer/theme.html" && request.method === "GET") {
    const selectedSample = sampleName(requestUrl.searchParams.get("sample") || "normal");
    serveThemeHtml(response, selectedSample);
    return;
  }
