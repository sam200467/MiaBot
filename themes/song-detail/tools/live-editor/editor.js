...
(() => {
  ...
  function applyCssNow() {
    ...
    removeRuntimeFontSizes(frameDocument);
    requestAnimationFrame(() => frameWindow.__FIT_SONG_THEME_TEXT__?.());
  }
  ...
  function removeRuntimeFontSizes(documentRoot) {
    documentRoot.querySelectorAll("[style]").forEach((element) => {
      if (element.style.fontSize) element.style.removeProperty("font-size");
    });
  }
  ...
  async function waitForThemeReady(frameWindow) {
    ...
    throw new Error("预览等待超时，请检查图片地址或主题脚本");
  }
  ...
  async function prepareFrame() {
    ...
    try {
      ...
      frameReady = true;
      frameWindow.__FIT_SONG_THEME_TEXT__?.();
  ...
  function refreshPreview() {
    ...
    elements.frame.src = "/song-detail/renderer/theme.html?sample=" + sample + "&liveEditor=1&t=" + Date.now();
  }
  ...
  function jumpToRule() {
    ...
    const index = elements.editor.value.indexOf(normalizedNeedle);
    if (index < 0) {
      showToast("没有找到该规则，可能已被重命名", true);
      return;
    }
    elements.editor.focus();
    elements.editor.setSelectionRange(index, index + normalizedNeedle.length);
  ...
  function setZoom(percent) {
    const value = Math.max(20, Math.min(100, Number(percent) || 50));
  ...
  function fitPreview() {
    const availableWidth = Math.max(320, elements.viewport.clientWidth - 38);
