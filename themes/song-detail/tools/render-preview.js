...
const PREVIEW_LOCK_PATH = path.join(RENDERER_DIR, ".preview-render.lock");
const SAMPLE_DATA_PATH = path.join(THEME_DIR, "examples", "preview-data.js");
const DEFAULT_OUTPUT = path.join(THEME_DIR, "previews", "song-detail-preview.png");
...
async function main() {
  ...
  const previewDataPath = path.resolve(process.argv[3] || SAMPLE_DATA_PATH);
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  let previewLock;
  try {
    previewLock = fs.openSync(PREVIEW_LOCK_PATH, "wx");
  } catch (error) {
    if (error?.code === "EEXIST") throw new Error("单曲主题预览正在生成，请勿并发运行 render-preview.js");
    throw error;
  }
  fs.copyFileSync(previewDataPath, DATA_JS_PATH);
  ...
  try {
    ...
    console.log("已验证固定五行、缺谱横杠、字段对齐与长文本边界。");
  } finally {
    cdp?.close();
    ...
    try { if (previewLock !== undefined) fs.closeSync(previewLock); } catch {}
    try { fs.rmSync(PREVIEW_LOCK_PATH, { force: true }); } catch {}
  }
}
