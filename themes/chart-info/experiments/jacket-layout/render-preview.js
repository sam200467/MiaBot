...
const sourceData = path.join(root, "preview-data.js");
const runtimeData = path.join(renderer, "preview-data.js");
const output = path.join(root, "jacket-layout-preview.png");
const browser = [
...
fs.copyFileSync(sourceData, runtimeData);
const port = 9733 + Math.floor(Math.random() * 300);
const userDataDir = path.join(os.tmpdir(), `chart-info-jacket-layout-${process.pid}`);
...
(async () => {
  ...
  try {
    ...
    if (!state?.ready) throw new Error("等待曲绘侧栏版渲染超时");
    ...
    try { fs.rmSync(runtimeData, { force: true }); } catch {}
  }
