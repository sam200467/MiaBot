...
(() => {
  ...
  const THEME_URL = "/chart-info/renderer/theme.html";
  const elements = {
  ...
  async function waitForThemeReady(frameWindow) {
    ...
    throw new Error("预览等待超时，请检查图片路径或主题脚本");
  }
  ...
  async function prepareFrame() {
    ...
    try {
      ...
      const themeLink = [...frameDocument.querySelectorAll("link[rel=stylesheet]")]
        .find((link) => new URL(link.href).pathname.endsWith("/theme.css"));
  ...
  function refreshPreview() {
    frameReady = false;
    elements.frame.src = THEME_URL + "?liveEditor=1&t=" + Date.now();
  }
  ...
  async function loadSavedCss(showMessage) {
    ...
    if (!response.ok) throw new Error("读取 themes/chart-info/renderer/theme.css 失败");
    savedCss = await response.text();
    elements.editor.value = savedCss;
    markDirty();
    updateCursorPosition();
    scheduleApply();
    if (showMessage) showToast("已恢复磁盘中的正式 theme.css");
  }
  ...
  async function saveCss() {
    ...
    try {
      ...
      markDirty();
      showToast("已保存到 themes/chart-info/renderer/theme.css");
    } catch (error) {
      markDirty();
      showToast(error.message || String(error), true);
    } finally {
  ...
  window.addEventListener("resize", () => { if (autoFit) fitPreview(); });
