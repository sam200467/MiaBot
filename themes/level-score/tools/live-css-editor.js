...
const SAMPLES = Object.freeze({
  full: path.join(THEME_DIR, "examples", "preview-data.js"),
  last: path.join(THEME_DIR, "examples", "preview-last-page.js"),
});
const SAMPLE_CANVASES = Object.freeze({
  full: { width: 1440, height: 2981 },
  last: { width: 1440, height: 1080 },
});
...
function safeStaticPath(urlPath) {
  ...
  const target = path.resolve(THEMES_ROOT, relative);
...
function sampleName(value) {
  return Object.hasOwn(SAMPLES, value) ? value : "full";
}

function serveThemeHtml(response, selectedSample, liveCss) {
  const html = fs.readFileSync(THEME_HTML_PATH, "utf8");
  const previewScript = "/__preview_data__.js?sample=" + encodeURIComponent(selectedSample);
  const editorMarker = "<script>window.__LIVE_CSS_EDITOR__ = true;</script>";
  let result = html.replace(
    '<script src="preview-data.js"></script>',
    editorMarker + "\n  <script src=\"" + previewScript + "\"></script>",
  );
  if (typeof liveCss === "string") {
    const safeCss = liveCss.replace(/<\/style/gi, "<\\/style");
    result = result.replace("</head>", "  <style id=\"__export_css__\">" + safeCss + "</style>\n</head>");
  }
  send(response, 200, result, "text/html; charset=utf-8");
}

function waitForChild(command, args) {
  return new Promise((resolve, reject) => {
    const child = spawn(command, args, { stdio: "ignore", windowsHide: true });
    child.once("error", reject);
    child.once("exit", (code) => {
      if (code === 0) resolve();
      else reject(new Error("浏览器截图进程退出，错误码 " + code));
    });
  });
}

async function capturePreview(selectedSample, css) {
  const browser = BROWSER_CANDIDATES.find((candidate) => candidate && fs.existsSync(candidate));
  if (!browser) throw new Error("未找到 Chrome 或 Edge，无法导出 PNG");
  const canvas = SAMPLE_CANVASES[selectedSample] || SAMPLE_CANVASES.full;
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "level-score-css-export-"));
  const outputPath = path.join(tempDir, "preview.png");
  const token = crypto.randomUUID();
  exportStyles.set(token, css);
  try {
    const port = server.address().port;
    const previewUrl = "http://" + HOST + ":" + port
      + "/level-score/renderer/theme.html?sample=" + encodeURIComponent(selectedSample)
      + "&exportCss=" + encodeURIComponent(token);
    await waitForChild(browser, [
      "--headless=new",
      "--disable-gpu",
      "--hide-scrollbars",
    ...
    if (!fs.existsSync(outputPath)) throw new Error("浏览器没有生成 PNG 文件");
    return fs.readFileSync(outputPath);
  } finally {
    exportStyles.delete(token);
...
async function handleRequest(request, response) {
  ...
  if (pathname === "/api/screenshot" && request.method === "POST") {
    try {
      const selectedSample = sampleName(requestUrl.searchParams.get("sample") || "full");
      const css = await readRequestBody(request);
      if (css.includes("\u0000")) throw new Error("CSS 中包含无效的空字符");
      const image = await capturePreview(selectedSample, css);
      send(response, 200, image, "image/png", {
        "Content-Disposition": "attachment; filename=level-score-" + selectedSample + "-preview.png",
      });
    } catch (error) {
  ...
  if (pathname === "/__preview_data__.js" && request.method === "GET") {
    const selectedSample = sampleName(requestUrl.searchParams.get("sample") || "full");
    serveFile(response, SAMPLES[selectedSample]);
    return;
  }

  if (pathname === "/level-score/renderer/theme.html" && request.method === "GET") {
    const selectedSample = sampleName(requestUrl.searchParams.get("sample") || "full");
    const exportCss = exportStyles.get(requestUrl.searchParams.get("exportCss") || "");
    serveThemeHtml(response, selectedSample, exportCss);
    return;
  }
  ...
  if (pathname.startsWith("/__editor__/")) {
    const relative = pathname.slice("/__editor__/".length);
    const target = path.resolve(EDITOR_DIR, relative);
...
server.listen(requestedPort(), HOST, () => {
  ...
  console.log("");
  console.log("等级成绩长图 CSS 实时调试器已启动");
  console.log("地址：" + url);
